...
\label{sec:specreq}
  ...
  \subsection{Use cases}
    ...
    \usecase % user registration
      {1}
      {Sign in of User to \texttt{Data4Help}}
      {User}
      ...
      {\uclist{
        \item[1] The field \texttt{email} is invalid or corresponds to an email that already exists in the system database; the field \texttt{fiscal code} is not valid; the \texttt{password} fields don't correspond. 
...
The system notifies the issue to the user and the \textbf{Flow of Events} returns to 2, erasing invalid fields
    ...
    \usecase % tp registration
      {2}
      {Sign in of Third party to \texttt{Data4Help}}
      {Third party}
      ...
      {\uclist{
        \item[1] The field \texttt{email} is invalid or corresponds to an email that already exists in the system database; the \texttt{password} fields don't correspond. 
...
The system notifies the issue to the third party and the \textbf{Flow of Events} returns to 2, erasing invalid fields
    ...
    \usecase % user login
      ...
      {\uclist{
        \item The system discovers that field \texttt{email} is invalid or doesn't corresponds to an email that already exists in the system database.
        \item The system discovers that field  \texttt{password} doesn't correspond to the one paired with the email.
...
{}
    ...
    \usecase % call ambulance
      ...
      {\uclist{
        \item[1] The SOS system is not reachable; \textbf{Flow of Events} rollbacked to 3
      }}
{The system has to make the call to SOSSystem within 5 seconds from the moment when it detectes that the user is in health danger}
      {}
    ...
    \usecase % single user request
      ...
      {\uclist{
        \item[1] The system doesn't find any user whose fiscal code correspond to the one inserted by the third party; \textbf{Flow of Events} rollbacked to 3
        \item[2] User declines the request; the system notifies to the third party that is not allowed to exploit user's data
      }}
      {}     
      {}
   ...
   \usecase % anonymous group request
      ...
      {\uclist{
        \item[1] The system doesn't find enough data entries that match the request filters; the system notifies to the third party that its request cannot be fulfilled
      }}
      {}
      {}
    ...
    \usecase % graphical filters
      ...
      {\uclist{
        \item[1] User inserted a non valid time period; the system notifies the user of the error; \textbf{Flow of Events} rollbacked to 4
      }}
      {}
      {}
