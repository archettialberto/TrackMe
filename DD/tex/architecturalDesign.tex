...
\label{sec:arcdes}
  ...
  \label{sec:deplview}
    ...
    We decided to deploy our system exploiting the services provided by Amazon AWS. Amazon AWS hides most of the back-end complexity in terms of scalability, reliability and fault tolerance of the system. It automatically handles server calls in a distributed way, by just providing the code we want to run.

    It is important to note that in the other parts of this document we didn't treat the Application Server as a physical node, but more as a logical node. In fact, the Application Server functionalities will be deployed by Lambda functions, that have no concept of node or server on which business logic is handled. By hiding the deployment details of the Application Server, we intended to isolate this section from the rest of the document, in order to ease future changes or additions.

    Figure~\ref{fig:Deployment} represents the deployment view of the system: each device refers to a different component of the system (Section~\ref{sec:compview}):
    \begin{description}
      \item[\texttt{Mobile device}] Runs the mobile Application of the presentation layer; the Application is compatible with Android and iOS operating systems, as they're the most common between TrackMe target clients
      \item[\texttt{PC}] Runs the CLI Application of the presentation layer; the Application is written in Java and is compatible with most desktop operating systems
      ...
      \item[\texttt{AWS Lambda}] Platform where it is possible to upload application code (written in one of the supported languages such as Node.js, Java, Go, C and Python) which will be run in an environment called Lambda function; every Lambda function is stateless (Section~\ref{sec:stylesandpatterns}), meaning that it is completely independant from other functions and can be executed in a distributed and very efficient way; AWS Lambda autonomously manages memory, CPU, network and other resources, so we are responsible only for our code
      \item[\texttt{Amazon RDS}] Amazon Relational Database Service provides an easy, cost-efficient, scalable database in the cloud which implements the data layer; Amazon RDS autonomously performs all the administration tasks such as database setup, patching and backups: we can focus only on how should the database behaviour be and not how can it be achieved
    \end{description}
  ...
  \label{sec:stylesandpatterns}
    ...
    \begin{description}
      ...
      \begin{enumerate}
      \item The client asks whether the server can accept requests or not at that specific moment
      \item The server accept to receive requests
      \item The third party ask for 1500 users weights and heartbeats since 01/2018
      \item The server denies the request because 1500 users with those characteristics cannot be found
      \end{enumerate}

      After fourth step due to stateless communication if the third party wants to make another request has repeat the procedure from the first step

      \item[Query API] Exploits HTTP requests and a Query parameter named Action to grant communication between Application Server and Database. Each Query request must include some common parameters to handle authentication and selection of an action.
