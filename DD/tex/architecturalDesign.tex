...
\begin{itemize}
  \item Mobile device:runs client application which performs only Presentation Layer functionalities given that a Thin Client architectural style is chosen(~\ref{sec:stylesandpatterns}). 
  \item PC: performs only Presentation Layer functionalities thanks to a proper CLI
\end{itemize}
...
\begin{itemize}
  \item AWS Lambda: computer platform where it is possible to upload application code (provided that is written in one of the supported languages such as Node.js, Java, Go , C and Python)which will be run in an environment called Lambda function. This lambda function is stateless and given that AWS Lambda autonomously manages memory, CPU, network, and other resources we are responsible only for our code; we have not to consider some critical aspects such as availability or load balancing. Moreover a stateless approach is coherent with the use of REST for communication as says ~\ref{sec:stylesandpatterns}).
  For all these reasons we choose AWS Lambda to implement Business Layer functionalities.
  This component can interact with both Presentation Layer and Data Layer(provided that)
